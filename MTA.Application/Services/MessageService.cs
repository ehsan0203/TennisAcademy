...
public class MessageService : IMessageService
{
    ...
    public async Task<PaginatedResult<MessageDto>> GetAllAsync(int page = 1, int pageSize = 10, string? searchTerm = null, int? ticketId = null, int? senderId = null, bool? isRead = null)
    {
        ...
        // Get total count
        var totalCount = await _unitOfWork.Repository<Message>().CountAsync(query);

        // Apply pagination
        var messages = await _unitOfWork.Repository<Message>().GetPagedAsync(query, page, pageSize);
    ...
    {
        var messages = await _unitOfWork.Repository<Message>().GetAllAsync(m => m.TicketId == ticketId);
    ...
    {
        var messages = await _unitOfWork.Repository<Message>().GetAllAsync(m => m.SenderId == senderId);
        return messages.Select(m => _mapper.Map<MessageDto>(m));
    }
    ...
    {
        var messages = await _unitOfWork.Repository<Message>().GetAllAsync(m => !m.IsRead);
        return messages.Select(m => _mapper.Map<MessageDto>(m));
    }
    ...
    /// </summary>
    public async Task<MessageDto> UpdateAsync(int id, MessageDto messageDto)
    {
